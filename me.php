                    ...
                    <?php foreach ($tasks as $task): ?>
                      <tr>
                        ...
                        <td>     <?php echo $task['profession']; ?> 
                            ...
                            <form method="post" class="updateProfessionForm">
                            <select class="form-control profession-dropdown" name="profession">
                            <option  > </option>
                                ...
                                <input type="hidden" name="profession_id" value="<?php echo $task['user_id']; ?>">
                                <button type="button" class="btn btn-success btn-sm update-button" data-profession-id="<?php echo $task['user_id']; ?>">updater</button>
                            </form>
                       </td>
                        ...
                        <td>
                            <form method="post" class="deleteForm">
                                ...
                                <button type="button" class="btn btn-danger btn-sm delete-button" data-task-id="<?php echo $task['user_id']; ?>">Delete User</button>
                            </form>
                        </td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.update-button').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            alert(userIdname+selectedProfession );
...
<script>
    function deleteTask(taskId) {
        var formData = new FormData();
        formData.append('user_id', taskId);

        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'process/deleteUser.php', true);
        xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 300) {
                var toastContainer = document.getElementById('toast-container');
                toastContainer.textContent = xhr.responseText;
